...
# Rutas de entrada
ruta_nulo = "...NULO_resumen_clade_model_completo.tsv"
ruta_alt = ".../resumen_clade_model_completo.tsv"
...
# 🧪 Filtrar genes con omega confiables (<10) en class2
...
# Corrección FDR Benjamini-Hochberg con clasificación flexible
...
# Guardar tabla completa y por categoría
...
plt.title("Distribución de p-valores (modelo clade vs nulo)")
plt.xlabel("p-valor")
...
# 📊 Gráfico adicional: distribución de omega (class2) en genes excluidos
...
plt.title("Distribución de omega (class2) en genes excluidos por omega ≥ 10")
plt.ylabel("ω (dN/dS)")
plt.xlabel("Clado")
plt.tight_layout()
...
print(f"Genes después del filtro (omega < 10 en class2): {len(df_filtrado)}")
print(f"Genes excluidos por tener omega ≥ 10 en alguna rama class2: {len(df_excluidos)}")
